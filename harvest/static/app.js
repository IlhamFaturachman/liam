...
const elements = {
    ...
    closeSqlModalBtn: document.getElementById('closeSqlModalBtn'),
    copySqlFromModal: document.getElementById('copySqlFromModal'),
};
...
function updateUI(status) {
    ...
    elements.exportSqlBtn.disabled = success === 0;
    elements.exportJsonBtn.disabled = success === 0;
    elements.copySqlBtn.disabled = success === 0;
...
// Export SQL
elements.exportSqlBtn.addEventListener('click', () => {
...
// Copy SQL (show modal with preview)
elements.copySqlBtn.addEventListener('click', async () => {
    try {
        const resp = await fetch('/api/export/sql/preview');
        const data = await resp.json();
        if (!resp.ok) {
            alert(data.error);
            return;
        }
        elements.sqlPreview.textContent = data.sql;
...
// Modal close
elements.closeSqlModal.addEventListener('click', () => {
    elements.sqlModal.style.display = 'none';
});
elements.closeSqlModalBtn.addEventListener('click', () => {
    elements.sqlModal.style.display = 'none';
});

// Copy SQL from modal
elements.copySqlFromModal.addEventListener('click', async () => {
    try {
        // Fetch full SQL (not just preview)
        const resp = await fetch('/api/export/sql');
        const text = await resp.text();
        await navigator.clipboard.writeText(text);
        elements.copySqlFromModal.textContent = 'Copied!';
        setTimeout(() => {
            elements.copySqlFromModal.textContent = 'Copy to Clipboard';
        }, 2000);
    } catch (err) {
        // Fallback: copy preview
        const text = elements.sqlPreview.textContent;
        await navigator.clipboard.writeText(text);
        elements.copySqlFromModal.textContent = 'Copied (preview)!';
        setTimeout(() => {
            elements.copySqlFromModal.textContent = 'Copy to Clipboard';
        }, 2000);
    }
