...
func (c *ClaudeCode) Apply(cfg ToolConfig) error {
	configPath := ExpandHome(c.ConfigPath())
	if err := EnsureDir(c.ConfigPath()); err != nil { return err }
	if err := BackupFile(c.ConfigPath()); err != nil { return err }
	...
	if env == nil { env = map[string]interface{}{} }
	env["ANTHROPIC_BASE_URL"] = cfg.BaseURL
	env["ANTHROPIC_AUTH_TOKEN"] = cfg.APIKey
	if m, ok := cfg.Models["opus"]; ok && m != "" { env["ANTHROPIC_DEFAULT_OPUS_MODEL"] = m }
	if m, ok := cfg.Models["sonnet"]; ok && m != "" { env["ANTHROPIC_DEFAULT_SONNET_MODEL"] = m }
	if m, ok := cfg.Models["haiku"]; ok && m != "" { env["ANTHROPIC_DEFAULT_HAIKU_MODEL"] = m }
...
func (c *ClaudeCode) Reset() error {
	...
	if env, ok := cfgMap["env"].(map[string]interface{}); ok {
		...
		if len(env) == 0 { delete(cfgMap, "env") } else { cfgMap["env"] = env }
	}
...
func (c *ClaudeCode) Snippet(cfg ToolConfig) string {
	...
	if opus == "" { opus = "ag/claude-opus-4-6-thinking" }
	if sonnet == "" { sonnet = "ag/claude-sonnet-4-6" }
