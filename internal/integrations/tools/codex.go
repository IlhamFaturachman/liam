...
`, model, cfg.BaseURL)
	...
	if err := os.WriteFile(configPath, []byte(final), 0644); err != nil { return err }
...
func stripLiamBlocks(content string) string {
	...
	for i := 0; i < len(lines); i++ {
		...
		if trimmed == "[model_providers.liam]" { inLiamBlock = true; continue }
		if inLiamBlock {
		...
		if strings.HasPrefix(trimmed, "model =") || strings.HasPrefix(trimmed, "model=") { continue }
		if strings.HasPrefix(trimmed, "model_provider =") || strings.HasPrefix(trimmed, "model_provider=") { continue }
		out = append(out, line)
	}
